...
function blm_bloom_post_save( $post_id , $post, $update) {
	...
	$query = array(
		...
		'app_action' => 'post_add',
		'title' => base64_encode( get_the_title() ),
		'content' => base64_encode( get_the_excerpt() ),
		'category' => 'news',
		...
		'image_url' => rawurlencode( get_the_post_thumbnail_url() ),
...
function blm_bloom_saved_message() {
	...
	if( 1 === $response->code ) {
		...
		update_post_meta( $post_ID, 'blm_bloom_post_access_code', $response->data->access_code );
