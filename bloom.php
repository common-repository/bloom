...
/*
Plugin Name: Bloom
Plugin URI: https://wordpress.org/plugins/bloom/
...
along with Bloom. If not, see https://www.gnu.org/licenses/gpl-2.0.html.
*/
...
function blm_location_callback( $post ) {
	...
	$blm_google_api_key = esc_attr( get_option( 'blm_google_api_key' ) );
	$blm_post_access_code = get_post_meta( $post->ID, 'blm_bloom_post_access_code', true );
	...
	if( $blm_google_api_key ) {
		...
		if( $blm_address_components && $blm_post_access_code ) {
			@file_get_contents( 'http://api.bloom.li/embed/article/map?akey='.$blm_post_access_code.'&size=rect&zoom=12' );
		}
